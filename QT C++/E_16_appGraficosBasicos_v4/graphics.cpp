...
// AGREGAR LOS CAMBIOS
void graphics::espiral1(QPainter *canvas, QColor color)
...
void graphics::espiral2(QPainter *canvas, QColor color)
{
    int xc = canvas->window().width()/2,
    ...
    while (radio < 230)
    {
        th += 1.7;    // VALORES INICIAL : .1;        // 1.7 ; //UTILZIAR VALORES PROPORCIONALES
        radio += 2.5;    // VALOR INICIAL : 0.9    //2.5; // USUARIO
        x = radio*cos(th);
        y = radio*sin(th);
