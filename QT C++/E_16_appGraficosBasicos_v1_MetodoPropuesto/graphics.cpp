...
void graphics::CirculoPitagoras(float xc, float yc, float r, QPainter *canvas)
{
    float x = xc - r, y1 = 0, y2 = 0;

    canvas->drawPoint(x, yc);
    while(x < xc+r)
    {
        y1 = yc + sqrt(pow(r, 2) - pow(x-xc,2));
        y2 = yc - sqrt(pow(r, 2) - pow(x-xc,2));
        canvas->drawPoint(x,y1);
        canvas->drawPoint(x,y2);
        x++;
    }
    canvas->drawPoint(x, yc);
